...
26. Implemente a função `tab(num, inicio, fim)` que esveva em tela a tabuada de um número que escreve no prompt de commando da linguagem R a tabuada do múmero passado como argumento à `num`. **Nota**: Os argumentos `inicio` e `fim` referem-se o início e fim da tabuada, ou seja, `tab(num = 1, inicio = 7, fim = 12)` deverá escrever no prompt:
